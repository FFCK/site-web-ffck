<?php
/*
  guard.class.php                                     v3.1.2 2016-08-23
  Author      : Stargraf
  Title       : Guardian
...
public function emailSubject($EmailSubject='', $SubjectNoChange='') {
  if ( !$EmailSubject ) {
    $EmailSubject='//////// GUARD: [SiteName] ALERT !';
  }
  if( !$SubjectNoChange ) {
    $SubjectNoChange='GUARD: [SiteName] OK :)';
  }
...
public function logFile($Basename='', $AppendSpec='_Y-m', $LogStyle='line') {
  ...
  if ( $Basename ) {
    ...
    if ( !$this->_isWritable($this->LogPathname) ) {
      die('twzFW: logFile '.$this->LogPathname.' is not writable');
    } // if die() is removed, set LogPathname to empty string (no logging)
  }
}
...
public function checkFiles() {
  ...
  if ( $ResultMessage <> '' ) {
    ...
    if ( !$AppName ) { $AppName='twzFW'; } // just in case

    // $EmailFrom = $AppName.'@'.$ServerName;
    $EmailFrom = 'guard@stargraf.com';
    ...
    if ( $this->optSendEmail ) {
      $EmailHeaders = array('From'=>"Guard Stargraf <$EmailFrom>", 'Cc'=>'', 'Bcc'=>$this->optEmailBcc, 'Reply-To'=>'');
      ...
      echo "<b>from</b> $EmailFrom <b>to</b> ".$this->optEmailTo.' <b>Subject:</b> '.$this->optEmailSubject."<hr />\r\n".'<pre>'.$ResultMessage.'</pre>';
    }
  }
}
...
public function getFilesInfo($AsArray=false, $Refresh=false) {
  if( $Refresh ) { $this->_rebuildSavefile(false); }
  if ( $AsArray ) { return $this->_getSavedInfo(); }
  elseif ( is_file($this->optSaveFile) ) { return file_get_contents($this->optSaveFile); }
  else { return ''; }
}
...
protected function _checkVars() {
  if ( is_string($this->optCheckFolder) ) {
    $this->optCheckFolder=array($this->optCheckFolder);
  } // optCheckFolder must be an array
...
protected function _getSavedInfo() {
  ...
  if ( is_file($this->optSaveFile) ) {
    ...
    if ( time() < strtotime(date('j M Y g:i:s a', $this->LastCheckInt).' +'.$this->optMinInterval) ) { die; } // ABORT
...
protected function _getCurrentInfo() {
  ...
  foreach($this->optCheckFolder as $ThisFolder) {
    ...
    $CurrentFiles=array_merge($CurrentFiles, $this->_fileList($ThisFolder, '', $ListFormat, $ListRecursion, $this->optExcludeFolder, $this->optFollowSymlinks));

    if(count($CurrentFiles[$ThisFolder])==0) { $this->Change[$ThisFolder][]='No files found'; }
  }
  return $CurrentFiles;
}
...
protected function _getResultMessage() {
  ...
  if($this->optShow['base']) {
    $RelFolder=false;
    foreach($this->optCheckFolder as $ThisFolder) {
      if($ThisFolder{0}!='/') { $RelFolder=true; break; }
    } // relative if doesn't start with slash
    if($RelFolder) {
      $ThisScript=$this->_getParentScriptPathname();
      if($ThisScript) {
        $ResultMessage.="<br>".$this->optShow['base'].' '.dirname($ThisScript);
      } // realpath() not required?
    }
  ...
  if ($this->optShow['time']) {
    $ResultMessage.="<br>"."time:"."<br>".$this->optShow['time'].' '.date('r'); // eg Thu, 18 Aug 2016 16:01:07 +1100
    $Timezone='';
    ...
    $ResultMessage.=' ('.trim($Timezone).')'. "<br><br>";
    unset($Timezone);
  }
  ...
  if(count($this->Change)>0) {
    ...
    if($this->optSuppressNewList and 0==count($this->PreviousFile)) {
      $ResultMessage .= "Full file list is suppressed. <br><br>";
    }
    else {
      $ResultMessage .= "<b>FILE CHANGES DETECTED:</b><br>";
      $ResultMessage .= ($this->TotalAdded + $this->TotalDeleted + $this->TotalUpdated)
      .' files affected'
      .' in ' . $FoldersAffected . " directories <br>"
      . '(' .$this->TotalAdded.' added, '.$this->TotalDeleted.' deleted, '.$this->TotalUpdated." updated) <br><br>"
      .$SayFilesizedelta;

      $ResultMessage .= "<b>FILE LAST SCAN:</b> $SayLastCheck <br><br>";
      $ResultMessage .= "<b>FILE CHANGES DETAILS:</b> ";
      
      foreach($this->Change as $ChFolder=>$ChList) {
        $FoldersAffected++;
        $ResultMessage.="<br>Directory $ChFolder <br>";
  ...
  elseif ( $this->optReportAlways ) {
    $ResultMessage.="<b>NO FILE CHANGES DETECTED</b><br><br><b>FILE LAST SCAN:</b> $SayLastCheck<br><br><b>FILE DIRECTORIES:</b><br> ";
    if ( count($this->optCheckFolder)>1 ) {
      $ResultMessage.="these directories:<br><br> ".implode("<br> ", $this->optCheckFolder)."<br>";
    }
    else {
      $ResultMessage.='directory '.$this->optCheckFolder[0]."<br>";
    }
  ...
  // End result message
  $ResultMessage .= '<br><br>Watching '.$this->TotalFileCount.' files in '.count($this->CurrentFile).' directories';
...
protected function _emailIsValid($EmailAddr,$Max=1) {
  $EmailRegex = '/^([_a-z0-9-]+)(\.[_a-z0-9-]+)*@([a-z0-9-]+)(\.[a-z0-9-]+)*(\.[a-z]{2,4})$/i'; $Email=explode(',',$EmailAddr);
  if ( count($Email) > $Max ) return false;
  foreach($Email as $ThisEmail) {
    if ( !preg_match($EmailRegex, $ThisEmail) ) return false;
  }
  return true;
}

protected function _isWritable($Pathname) {
  if ( is_file($Pathname) and is_writable($Pathname) ) { return true; }
  elseif ( !is_file($Pathname) and is_writable(dirname($Pathname)) ) {
