...
class EvenementDetail extends MyPage
{
	function Footer()
	{
	...
	?>
		...
		<!-- CSS -->
		<link href="<?php echo $this->m_directory;?>calendrier.css" rel="stylesheet">
	...
	function Content()
	{
		$codex = $this->GetParam('codex');

		?>
		<div id='container_formation_detail' class='well'>
		<?php $this->Affiche($codex); ?>
		</div>
		<?php
	}
	...
	{
		$fichiers = $xmlDoc->getElementsByTagName('fichier');
		$i = 0;
		?>
		<table>
		<?php
	...
	public static function Affiche($codex)
	{

    $wsEndpoint = "http://" . getenv("OBS_USER") . ":" . getenv("OBS_PASSWORD") . "@" . getenv("OBS_HOST") . "/wsprivate";

		$url = $wsEndpoint . "/manifestations/detailManifestation?code=";
		$url .= $codex;

		$xmlDoc = new DOMDocument();
		$xmlDoc->load($url);

		$dateDebut = xmlGetValue($xmlDoc, 'debut');
		$dateFin = xmlGetValue($xmlDoc, 'fin');

		$xmlManifestation = xmlGetElement($xmlDoc, 'manifestation', 2);
		$xmlStructure = xmlGetElement($xmlManifestation, 'structure',1);
		$xmlResponsable = xmlGetElement($xmlManifestation, 'responsable',1);
		$xmlLieu = xmlGetElement($xmlManifestation, 'lieuEvenement',1);
		$longitude = xmlGetValue($xmlLieu, 'longitude');

		?>
		<h2><?php echo xmlGetValue($xmlDoc,'libelleEvenement');?></h2>
		<div class="table-responsive">
		<table class="table table-striped table-bordered table-hover table-condensed">
			<tbody>
				<tr><td>Codex</td><td><?php echo $codex;?></td></tr>
				<tr><td>Caractéristiques</td><td><?php echo xmlGetValue($xmlDoc, 'libelleCategorie')." - ".xmlGetValue($xmlDoc, 'libelleActivite')." - ".xmlGetValue ($xmlDoc, 'libelleNiveau');?></td></tr>
				<?php
					if ($dateDebut == $dateFin) {echo "<tr><td>Date</td><td>".$dateDebut."</td></tr>";}
					else {echo "<tr><td>Date</td><td>du ".$dateDebut." au ".$dateFin."</td></tr>";}
				?>
				<tr><td>Site web</td><td><a href="<?php echo xmlGetValue($xmlDoc, 'siteWeb');?>"  target="_blank"><?php echo xmlGetValue($xmlDoc, 'siteWeb');?></a></td></tr>
				<tr><td>Lieu</td><td><?php echo xmlGetValue($xmlLieu, 'ville').", ".xmlGetValue($xmlLieu, 'pays');?></td></tr>
				<tr><td>Coordonnées GPS</td><td>
						<?php
						if ($longitude != "") {
							echo "long : ".$longitude." - lat : ".xmlGetValue($xmlLieu, 'longitude');
						}
						else {
				...
				<tr><td>Organisateur</td><td><?php echo xmlGetValue($xmlStructure, 'nom');?></td></tr>
				<tr><td>Responsable</td><td><?php echo xmlGetValue($xmlResponsable, 'civilite')." ".xmlGetValue($xmlResponsable, 'prenom')." ".xmlGetValue($xmlResponsable, 'nom');?></td></tr>
				<tr><td>Email</td><td><a href="mailto:<?php echo xmlGetValue($xmlLieu, 'email');?>"><?php echo xmlGetValue($xmlLieu, 'email');?></a></td></tr>
				<tr><td>Téléphone</td><td><?php echo xmlGetValue($xmlLieu, 'telephone1');?></td></tr>
				<tr><td>Mobile</td><td><?php echo xmlGetValue($xmlLieu, 'mobile1');?></td></tr>

				<tr><td colspan="2">&nbsp;</td></tr>
				<tr><td>Niveau Pagaie Couleur minimum</td><td><?php echo xmlGetValue($xmlDoc, 'libellePagaieMinimum');?></td></tr>
				<tr><td>Document(s) Joint(s)</td><td><?php echo EvenementDetail::PJ($xmlDoc);?></td></tr>
			</tbody>
		</table>
		</div>
		...
		<div class="desc">
			<p>
				<strong class="hed"><?php echo xmlGetValue($xmlDoc,'libelleEvenement');?></strong>
				<span class="des"><?php echo xmlGetValue($xmlLieu, 'ville').", ".xmlGetValue($xmlLieu, 'pays');?></span>
			</p>
		</div>
		<span class="_start"><?php echo $dateDebut;?></span>
		<span class="_end"><?php echo $dateFin;?></span>
		<span class="_zonecode">40</span>
		<span class="_summary"><?php echo xmlGetValue($xmlDoc,'libelleEvenement');?></span>
		<span class="_description"><?php echo xmlGetValue($xmlDoc, 'libelleCategorie')." ".xmlGetValue($xmlDoc, 'libelleActivite')." ".xmlGetValue($xmlDoc, 'libelleNiveau')." N°".$codex;?></span>
		<span class="_location"><?php echo xmlGetValue($xmlLieu, 'ville').", ".xmlGetValue($xmlLieu, 'pays');?></span>
	...
	function Script()
	{
		parent::Script();
		?>
		<script type="text/javascript" src="./js/evenement_detail.js" ></script>
