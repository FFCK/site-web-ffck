...
/*global wp shepherd rmlSticky rmlRefreshIt rmlToggleTreeStatus rmlUpdateTreeCollapse rmlPrepareUpload rmlUploadProgress rmlUploadSuccess rmlUpdateActiveType rmlPrepareAddForm  rmlToggleEditMode RMLisDefined rmlDraggable*/
...
jQuery(document).ready(function($) {
    // Info
    var infoLinksHelpClass = "active-show";
    if (window.localStorage) {
        infoLinksHelpClass = window.localStorage.getItem('rml-info-link-info');
        if (!RMLisDefined(infoLinksHelpClass)) {
            infoLinksHelpClass = "active-hide";
        }
    }
    $("#rml-info-link-info, #rml-info").addClass(infoLinksHelpClass);
    $(document).on('click', "#rml-info-link-info", function(e) {
        var removeClass = "active-hide", addClass = "active-show";
        if ($(this).hasClass("active-show")) {
            removeClass = "active-show";
            addClass = "active-hide";
        }
        $(this).removeClass(removeClass).addClass(addClass);
        $('#rml-info').removeClass(removeClass).addClass(addClass);
        if (window.localStorage) {
            window.localStorage.setItem('rml-info-link-info', addClass);
        }
        rmlSticky();
        e.preventDefault();
        return false;
    ...
    if ($("body").hasClass("wp-admin")
        ...
        /**
         * Handler for page title action helper.
         */
        if (!('ontouchstart' in document.documentElement)) {
            $(document).on("mouseenter", "a[data-helper]", function() {
                $('.page-title-action-helper[data-helper="' + $(this).attr("data-helper") + '"]').show();
            }).on("mouseleave", "a[data-helper]", function() {
                $('.page-title-action-helper[data-helper="' + $(this).attr("data-helper") + '"]').hide();
            });
        }else{
            $("body").addClass("rml-is-touch");
        }

        /**
         * Handler for clicking the button to create a new folder.
         */
        $(document).on("click touchstart", "#rml-add-new-folder, #rml-add-new-collection, #rml-add-new-gallery", function(e) {
            rmlPrepareAddForm($(this).attr("data-type"));
            e.preventDefault();
            ...
            if (slug == "/" || id == -1) {
                ...
                if (RMLisDefined(active[0]) && RMLisDefined(name) && name.length > 0) {
                    var cnt = active.find("span").html(),
                        ...
                        icons = "<i class=\"fa fa-folder-open\"></i><i class=\"fa fa-folder\"></i>";
                        
                    if (type == "1") {
                        icons = "<i class=\"mwf-collection\"></i>";
                    }else if (type == "2") {
                        icons = "<i class=\"mwf-gallery\"></i>";
                    }
                    
                    var newNameHTML = icons + name + "<span class=\"rml-cnt-" + cnt + "\">" + cnt + "</span>";
        ...
        $(document).on("click", ".rml-container .wrap.edit-mode button", function(e) {
            if ($(this).hasClass("button-primary")) {
                $(this).prepend('<i class="fa fa-circle-o-notch fa-spin" style="margin-right:5px;"></i>');
                    ...
                    jQuery.post(
                        ...
                        function(response){
                            if (response.success) {
                                ...
                                $(".rml-container .wrap.edit-mode button.save-sort i").remove();
                                alert(response.data);
                                rmlToggleEditMode(false);
                                $(".rml-container .rml-root-list").html(window.rmlOldHTML);
                            }
                        }
                    );
                }else{
