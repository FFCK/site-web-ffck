		...
		<div class="row">
			...
			<div class="col-md-<?php echo $map ? 6 : 12; ?> col-sm-12">
				<div class="listing-contact-overview">
					...
					?>
					<?php if(get_post_custom_values("mobile_site")[0] != ''){ ?>
					<div class="job_listing-url job_listing-portable">
              <span itemprop="url"><a href="<?php echo get_post_custom_values("mobile_site")[0]; ?>" rel="nofollow" target="_blank"><?php echo get_post_custom_values("mobile_site")[0]; ?></a></span>
          </div>
					<?php } ?>
					<?php
						if ( $web ) :
          ...
          ?>
					<?php if(get_post_custom_values("club_email")[0] != ''){ ?>
          <div class="job_listing-url job_listing-email">
              <span itemprop="url"><a href="mailto:<?php echo get_post_custom_values("club_email")[0]; ?>" rel="nofollow" target="_blank"><?php echo get_post_custom_values("club_email")[0]; ?></a></span>
          </div>
					<?php } ?>
          <?php do_action( 'listify_widget_job_listing_map_after' ); ?>
					</div>
				</div>
			</div>
			<?php endif; ?>
