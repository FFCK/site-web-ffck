...
class Listify_Walker_Tags_Checklist extends Walker_Category_Checklist {
	...
	function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ) {
      ...
      if ( in_array( $category->term_id, $selected_cats ) == true ) {
        $output .= sprintf(
        ...
        $output = str_replace("HAN-MOT", "Moteur", $output);
        $output = str_replace("HAN-MEN", "Mental", $output);
        $output = str_replace("HAN-AUD", "Auditif", $output);
        $output = str_replace("HAN-VIS", "Visuel", $output);
      }
    ...
    if ($output == '') {
      $output .= "Aucun accueil pris en charge.";
    }
		return '';
	}
}
